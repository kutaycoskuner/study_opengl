...
bool Application::initialize(const ConfigData& config)
{
    ...
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, sample_count, GL_RGBA16F, display_width, display_height,
                            GL_TRUE);
    glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, tex_illumination_msaa_color,
                           0);
    ...
    for (unsigned int i = 0; i < 2; i++)
    {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D, tex_illumination_hdr[i], 0);
    }
    ...
    for (unsigned int i = 0; i < 6; ++i)
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_DEPTH_COMPONENT, shadowmap_resolution_x,
                     shadowmap_resolution_y, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
...
bool Application::load(const ConfigData& config)
{
    ...
    glBufferData(GL_UNIFORM_BUFFER, range * sizeof(Mat4), NULL,
                 GL_STATIC_DRAW);  // allocate size of mat4 x2 byte of memory
...
void Application::loadSceneData(const ConfigData& config)
{
    ...
    if (scene_number == 0)
        ...
        active_scene = new MultipleLightsTestScene;
    else if (scene_number == 2)
        active_scene = new ImportModelTestScene;
    else if (scene_number == 3)
        active_scene = new OutlinerTestScene;
    else if (scene_number == 4)
        active_scene = new BlendingTestScene;
    else if (scene_number == 5)
        active_scene = new FaceCullingTestScene;
    else if (scene_number == 6)
        active_scene = new FrameBufferTestScene;
    else if (scene_number == 7)
        active_scene = new CubemapTestScene;
    else if (scene_number == 8)
        active_scene = new AdvancedGLSLTestScene;
    else if (scene_number == 9)
        active_scene = new GeoShaderTestScene;
    else if (scene_number == 10)
        active_scene = new InstancingTestScene;
    else if (scene_number == 11)
        active_scene = new AntiAliasingTestScene;
    else if (scene_number == 12)
        active_scene = new BlinnPhongTestScene;
    else if (scene_number == 13)
        active_scene = new GammaCorrectionTestScene;
    else if (scene_number == 14)
        active_scene = new ShadowsTestScene;
    else if (scene_number == 15)
        active_scene = new PointShadowsTestScene;
    else if (scene_number == 16)
        active_scene = new NormalMapTestScene;
    else if (scene_number == 17)
        active_scene = new ParallaxTestScene;
    else if (scene_number == 18)
        active_scene = new HDRTestScene;
    else if (scene_number == 19)
...
void Application::loadTextures()
{
    ...
    for (auto it = active_scene->texture_names.begin(); it != active_scene->texture_names.end(); ++it)
    {
        ...
        PathAfterDirectory::texture_paths[texture_name].roughness != ""
            ? texture_set.roughness =
                  createTexture(data_dir_path, PathAfterDirectory::texture_paths[texture_name].roughness)
        ...
        PathAfterDirectory::texture_paths[texture_name].specular != ""
            ? texture_set.specular =
                  createTexture(data_dir_path, PathAfterDirectory::texture_paths[texture_name].specular)
            : texture_set.specular = 0;

        PathAfterDirectory::texture_paths[texture_name].emission != ""
            ? texture_set.emission =
                  createTexture(data_dir_path, PathAfterDirectory::texture_paths[texture_name].emission)
            : texture_set.emission = 0;
        textures[texture_name] = texture_set;

        PathAfterDirectory::texture_paths[texture_name].displacement != ""
            ? texture_set.displacement =
                  createTexture(data_dir_path, PathAfterDirectory::texture_paths[texture_name].displacement)
...
void Application::loadMeshData()
{
    ...
    // glBufferData(GL_ARRAY_BUFFER, sizeof(VertexData::cube_vrts__pos_uv), VertexData::cube_vrts__pos_uv,
    // GL_STATIC_DRAW);
    ...
    // glBufferData(GL_ARRAY_BUFFER, sizeof(VertexData::cube_vrts__pos_norm_uv), VertexData::cube_vrts__pos_norm_uv,
    // GL_STATIC_DRAW); stride = 8;
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(VertexData::cube_inds__pos_norm_uv),
                 VertexData::cube_inds__pos_norm_uv, GL_STATIC_DRAW);
    ...
    for (auto it = PredefNameMaps::predef3d_namemap.begin(); it != PredefNameMaps::predef3d_namemap.end(); ++it)
    {
        ...
        glEnableVertexAttribArray(0);
        glVertexAttribPointer(0, it_obj.att_num_elements[0], GL_FLOAT, GL_FALSE, it_obj.stride * sizeof(float),
                              (void*)0);
        ...
        if (it_obj.att_num_elements.size() > 1)
        {
            ...
            glVertexAttribPointer(1, it_obj.att_num_elements[1], GL_FLOAT, GL_FALSE, it_obj.stride * sizeof(float),
                                  (void*)(att_size * sizeof(float)));
        }
        ...
        if (it_obj.att_num_elements.size() > 2)
        {
            ...
            glVertexAttribPointer(2, it_obj.att_num_elements[2], GL_FLOAT, GL_FALSE, it_obj.stride * sizeof(float),
                                  (void*)(att_size * sizeof(float)));
        }
    ...
    ////glBufferData(GL_ARRAY_BUFFER, sizeof(Vec2) * 100, &active_scene->computed_data.translations2d[0],
                                 ...
                                 bool tex)
{
    ...
    // att: vrtx
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, stride * sizeof(float), (void*)0);
    if (vrtx) glEnableVertexAttribArray(0);
    // att: texture
...
void Application::compute_QuadVrtxTangents()
{
    if (computed_vertex_array_obj == 0)
    {
        ...
        float quadVertices[] = {
            ...
            uv1.y,  tangent1.x, tangent1.y, tangent1.z, handedness,  // bitangent1.x, bitangent1.y, bitangent1.z,
            pos2.x, pos2.y,     pos2.z,     nm.x,       nm.y,       nm.z, uv2.x,
            uv2.y,  tangent1.x, tangent1.y, tangent1.z, handedness,  // bitangent1.x, bitangent1.y, bitangent1.z,
            pos3.x, pos3.y,     pos3.z,     nm.x,       nm.y,       nm.z, uv3.x,
            uv3.y,  tangent1.x, tangent1.y, tangent1.z, handedness,  // bitangent1.x, bitangent1.y, bitangent1.z,

            pos1.x, pos1.y,     pos1.z,     nm.x,       nm.y,       nm.z, uv1.x,
            uv1.y,  tangent2.x, tangent2.y, tangent2.z, handedness,  // bitangent2.x, bitangent2.y, bitangent2.z,
            pos3.x, pos3.y,     pos3.z,     nm.x,       nm.y,       nm.z, uv3.x,
            uv3.y,  tangent2.x, tangent2.y, tangent2.z, handedness,  // bitangent2.x, bitangent2.y, bitangent2.z,
            pos4.x, pos4.y,     pos4.z,     nm.x,       nm.y,       nm.z, uv4.x,
            uv4.y,  tangent2.x, tangent2.y, tangent2.z, handedness  // bitangent2.x, bitangent2.y, bitangent2.z
        };
        // configure plane VAO
...
void assignBuffer(const float* objToDraw, const int sizeofObjToDraw, const unsigned int& inptLayout,
                  const unsigned int& vrtxBuffer)
{
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0,
                          (void*)0);  // because the vertex data is tightly packed we can also specify 0 as the vertex
...
void Application::drawScene(Uniforms& uni)
{
    ...
    if (!active_scene->scene_state.b_deferred_shading)
    {
        ...
        glStencilOp(GL_KEEP,    // stencil action if stencil test fails
...
void Application::drawShadowCubemap()
{
    ...
    // Right (+X face)
    mat_light_space_cubemaps[0] =
        shadow_proj * mat_utils::lookAtDirection(light_pos, Vec3(+1.0, 0.0, 0.0), Vec3(0.0f, -1.0f, 0.0f));  // Up is -Y

    // Left (-X face)
    mat_light_space_cubemaps[1] =
        shadow_proj * mat_utils::lookAtDirection(light_pos, Vec3(-1.0, 0.0, 0.0), Vec3(0.0f, -1.0f, 0.0f));  // Up is -Y

    // Top (+Y face)
    mat_light_space_cubemaps[2] =
        shadow_proj * mat_utils::lookAtDirection(light_pos, Vec3(0.0, +1.0, 0.0), Vec3(0.0f, 0.0f, -1.0f));  // Up is +Z

    // Bottom (-Y face)
    mat_light_space_cubemaps[3] =
        shadow_proj * mat_utils::lookAtDirection(light_pos, Vec3(0.0, -1.0, 0.0), Vec3(0.0f, 0.0f, +1.0f));  // Up is -Z

    // Front (+Z face)
    mat_light_space_cubemaps[4] =
    ...
    if (active_scene->scene_state.b_bloom)
    {
        ...
        for (unsigned int i = 0; i < amount; i++)
        {
            ...
            glBindTexture(
                GL_TEXTURE_2D,
                first_iteration
                    ? tex_illumination_hdr[1]
                    : tex_bloom[!horizontal]);  // bind texture of other framebuffer (or scene if first iteration)
            // bind vertex array
            glBindVertexArray(named_arrays.at("frame"));
...
void Application::drawSceneNode_primitive_shadows_dl(const Mat4& model_mat)
{
    ...
    for (int i = 0; i < element_count; i++)
    {
        ...
        const Mat4       model     = mat_utils::translation(Vec3(transform.position.x, transform.position.y,
                                                                 transform.position.z))  // 0, 3, 10 * cos(ss.time)
        ...
        //	glDrawElements(GL_TRIANGLES, sizeof(float) * PredefNameMaps::predef3d_namemap.at(name).num_elements,
        // GL_UNSIGNED_INT, 0);
        // }
...
void Application::setUniforms(Uniforms& uni)
{
    ...
    // assign shadow
    if (mat_light_space.size() > 0) active_shader->setMat4("light_space_matrix", mat_light_space[0]);
    ...
    // factors
    active_shader->setFloat("material.emission_factor",
                            active_scene->scene_state.emission_factor);  // material emission factor
...
void Application::drawSceneNodes_primitive(Uniforms& uni)
{
    ...
    for (int i = 0; i < element_count; i++)
    {
        ...
        glActiveTexture(GL_TEXTURE1);
        textures[texture_name].specular != 0 ? glBindTexture(GL_TEXTURE_2D, textures[texture_name].specular)
                                             : glBindTexture(GL_TEXTURE_2D, 0);

        glActiveTexture(GL_TEXTURE2);
        textures[texture_name].color != 0 ? glBindTexture(GL_TEXTURE_2D, textures[texture_name].normal)
                                          : glBindTexture(GL_TEXTURE_2D, 0);

        glActiveTexture(GL_TEXTURE3);
        textures[texture_name].color != 0 ? glBindTexture(GL_TEXTURE_2D, textures[texture_name].emission)
                                          : glBindTexture(GL_TEXTURE_2D, 0);

        glActiveTexture(GL_TEXTURE4);
        tex_shadow_maps.size() > 0 ? glBindTexture(GL_TEXTURE_2D, tex_shadow_maps[0]) : glBindTexture(GL_TEXTURE_2D, 0);

        glActiveTexture(GL_TEXTURE5);
        tex_shadow_cubemaps.size() > 0 ? glBindTexture(GL_TEXTURE_CUBE_MAP, tex_shadow_cubemaps[0])
        ...
        model               = mat_utils::translation(
                    Vec3(transform.position.x, transform.position.y, transform.position.z))  // 0, 3, 10 * cos(ss.time)
                * mat_utils::rotateX(toRadian(transform.rotation.x)) *
                mat_utils::rotateY(toRadian(transform.rotation.y)) *
                mat_utils::rotateZ(toRadian(transform.rotation.z)) *
                mat_utils::scale(transform.scale.x, transform.scale.y, transform.scale.z);
        ...
        else if (active_scene->predefined_scene_elements[i].element_bools.indexed)  // face culling
        {
            // Bind the index buffer
            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, lit_ebo);
            int count = PredefNameMaps::predef3d_namemap.at(name).num_elements /
                        PredefNameMaps::predef3d_namemap.at(name).stride;
...
void Application::drawSceneNodes_models(Uniforms& uni)
{
    ...
    for (int i = 0; i < active_scene->models.size(); i++)
    {
        ...
        if (active_scene->scene_state.draw_instanced)
        {
            ...
            //	{
            //		glBindBuffer(GL_UNIFORM_BUFFER, ubo_matrices);
            //		glBufferSubData(GL_UNIFORM_BUFFER, 2*sizeof(Mat4), size*sizeof(Mat4),
            //&active_scene->computed_data.mat4[ii*size].m[0][0]); 		glBindBuffer(GL_UNIFORM_BUFFER, 0);
            ...
            // glBindBuffer(GL_UNIFORM_BUFFER, ubo_matrices);
            // glBufferSubData(GL_UNIFORM_BUFFER, 2*sizeof(Mat4), 5000*sizeof(Mat4),
            // &active_scene->computed_data.mat4[0].m[0][0]); glBindBuffer(GL_UNIFORM_BUFFER, 0);
            //		active_scene->models[i].drawInstanced(*active_shader, active_scene->scene_state.instance_count);
            ...
            for (unsigned int i = 0; i < active_scene->models[0].meshes.size(); i++)
            {
                ...
                glDrawElementsInstanced(
                    ...
                    GL_UNSIGNED_INT, 0, static_cast<GLsizei>(active_scene->scene_state.instance_count));
...
void Application::updateScene()
{
    ...
    // animate?
    if (!scene_state.animate) return;
    if (save_frame) scene_state.time = 0.0f;
    active_scene->update();
}
...
unsigned int fourStageAnimation(const unsigned int& stage) { return stage == 4 ? 1 : stage + 1; }
