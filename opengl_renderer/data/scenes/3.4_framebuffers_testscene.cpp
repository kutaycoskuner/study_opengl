...
// ----------------------------------------------------------------------------
void Application::frameBuffersTestScene(Uniforms& uni)
{
	...
	// camera position
	ss.camera.position = Vec3(-2.97f, 3.52f, 6.02f);
	ss.camera.lookAtTarget(Vec3(0.0f, 0.0f, 0.0f));

	// draw 
	disableStencil();
	//drawSingleCube(lit_vao, "multiplelights", uni);
	drawGroundPlane(lit_vao, "multiplelights", uni);
	drawTwoCubes(lit_vao, "multiplelights", uni);
	//drawGroundPlane(lit_vao, "multiplelights", uni);


	// outline
	enableStencil();
	//drawTwoCubes(lit_vao, "stenciltesting", uni);
