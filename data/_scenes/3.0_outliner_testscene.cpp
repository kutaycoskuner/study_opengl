...
void OutlinerTestScene::loadData()
{
    ...
    for (int i = 0; i < predefined_scene_elements.size(); i++)
    {
        ...
        predefined_scene_elements[i].element_bools.stencil_testing = true;
    }

    // ---- define model paths
    model_paths = {std::string("models/testobject0_frustum/testobject.obj"),
    ...
    {
        scene_state.model_element_bools.push_back(ElementBools(false,  // wireframe_mode
                                                               true,   // depth testing
...
void OutlinerTestScene::update()
{
    ...
    for (int ii = 0; ii < point_lights.size(); ii++)
    {
        ...
        point_lights[ii].position =
            Vec3(distance_multiplier * cos(scene_state.time + 2 * PI / point_lights.size() * ii), 3.0f,
    ...
    for (int i = 0; i < predefined_scene_elements.size(); i++)
    {
        if (predefined_scene_elements[i].name == "ground_plane") continue;
        if (predefined_scene_elements[i].name == "light_placeholder") continue;
    ...
    for (int i = 0; i < predefined_scene_elements.size(); i++)
    {
        ...
        predefined_scene_elements[i].transform.position = Vec3(time_offset * x, y, time_offset * z);
    }
